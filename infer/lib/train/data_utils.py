...
class TextAudioLoaderMultiNSFsid(torch.utils.data.Dataset):
    ...
    def get_audio_text_pair(self, audiopath_and_text):
        ...
        emotion_feature = audiopath_and_text[5]

        phone, pitch, pitchf = self.get_labels(phone, pitch, pitchf)
        spec, wav = self.get_audio(file)
        dv = self.get_sid(dv)
        emotion_feature = torch.FloatTensor(np.load(emotion_feature))
        ...
        if len_phone != len_spec:
            ...
            emotion_feature = emotion_feature[:len_min, :]

        return (spec, wav, phone, pitch, pitchf, dv, emotion_feature)
...
class TextAudioCollateMultiNSFsid:
    ...
    def __call__(self, batch):
        ...
        for i in range(len(ids_sorted_decreasing)):
            ...
            emotion = row[6]
            emotion_padded[i, : emotion.size(0), :] = emotion  # 수정된 부분
...
class TextAudioLoader(torch.utils.data.Dataset):
    ...
    def _filter(self):
        ...
        audiopaths_and_text_new = []
        lengths = []
        for audiopath, text, dv in self.audioFpaths_and_text:
            if self.min_text_len <= len(text) and len(text) <= self.max_text_len:
