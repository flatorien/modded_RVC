...
def click_train(
    ...
    if if_f0_3:
        ...
        emo_dir = "%s/2c-emo" % (exp_dir)
        names = (
            set([name.split(".")[0] for name in os.listdir(gt_wavs_dir)])
            & set([name.split(".")[0] for name in os.listdir(feature_dir)])
            & set([name.split(".")[0] for name in os.listdir(f0_dir)])
            & set([name.split(".")[0] for name in os.listdir(f0nsf_dir)])
            & set([name.split(".")[0] for name in os.listdir(emo_dir)])
        )
    else:
    ...
    for name in names:
        if if_f0_3:
            opt.append(
                "%s/%s.wav|%s/%s.npy|%s/%s.wav.npy|%s/%s.wav.npy|%s|%s/%s.wav.npy"
                % (
                    gt_wavs_dir.replace("\\", "\\\\"),
                    name,
                    feature_dir.replace("\\", "\\\\"),
    ...
    if if_f0_3:
        for _ in range(2):
            opt.append(
                "%s/logs/mute/0_gt_wavs/mute%s.wav|%s/logs/mute/3_feature%s/mute.npy|%s/logs/mute/2a_f0/mute.wav.npy|%s/logs/mute/2b-f0nsf/mute.wav.npy|%s|%s/logs/mute/2c-emo/mute.wav.npy"
                % (now_dir, sr2, now_dir, fea_dim, now_dir, now_dir, spk_id5, now_dir)
            )
    else:
        for _ in range(2):
            opt.append(
                "%s/logs/mute/0_gt_wavs/mute%s.wav|%s/logs/mute/3_feature%s/mute.npy|%s|%s/logs/mute/2c-emo/mute.wav.npy"
                % (now_dir, sr2, now_dir, fea_dim, spk_id5, now_dir)
            )
    shuffle(opt)
    ...
    p.wait()
    return "Training completed. You can check the training log in the console or in the train.log file in the experiment folder."
